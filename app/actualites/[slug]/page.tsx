...
import { articlesActualites, getArticleActualites } from '@/lib/articles-actualites'
import { Clock, ArrowLeft, ArrowRight, Tag } from 'lucide-react'

type Props = { params: Promise<{ slug: string }> }

export async function generateStaticParams() {
  return articlesActualites.map((a) => ({ slug: a.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const article = getArticleActualites(slug)
  if (!article) return {}
  return {
    title: article.metaTitle,
    description: article.metaDescription,
    keywords: article.keywords,
    alternates: { canonical: `https://brocanteradar.fr/actualites/${slug}` },
    openGraph: {
      title: article.metaTitle,
      description: article.metaDescription,
      url: `https://brocanteradar.fr/actualites/${slug}`,
      type: 'article',
...
export default async function ArticleActualitePage({ params }: Props) {
  const { slug } = await params
  const article = getArticleActualites(slug)
  if (!article) notFound()

  const others = articlesActualites.filter((a) => a.slug !== slug).slice(0, 3)
      ...
      <main className="flex-1 bg-white">
        ...
        <section className="py-10 px-4 bg-[#EEF4FF] border-t border-blue-100">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-xl font-black text-[#0D1B4B] mb-6">Autres actualités brocante</h2>
